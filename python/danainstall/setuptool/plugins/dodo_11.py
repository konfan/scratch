...
install_template = {
        ...
        "copy files":makecopy('copy files', os.path.join(basedefs.PACKAGE_DIR, dodo_package),
           os.path.join(package_dest, dodo_package)),
        "unpack":makescript('unpack', 'cd %s && tar zxf %s'%(package_dest, dodo_package)),

        'install':makescript('install', "chmod a+x {0} && cd {1} && ./install.sh".format(
            os.path.join(package_dest, 'dodo', 'install.sh'),
            os.path.join(package_dest, 'dodo')))
        }
...
def buildplan(config):
    baseconfig(config)
    install_seq = core.SequenceTemplate('dodo_nodes', maketemplate())
    hosts = ['%s@%s'%(config.get('common', 'installuser'), x.strip()) for x in dodo_config['hosts']]
    return [core.ExecutePlan('dodo', install_seq, hosts)]
